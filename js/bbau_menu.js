...
// 3. CART SYSTEM LOGIC
// =========================================
let cart = JSON.parse(localStorage.getItem("bbau_cart")) || [];

function saveCart() {
    localStorage.setItem("bbau_cart", JSON.stringify(cart));
    updateCartBadge();
}
...
function generateOrderId() {
    return "BBAU-" + Date.now().toString().slice(-6);
}
...
function uploadToOrder(file) {
    ...
    // 2. Check if phone number is exactly 10 digits
    ...
    reader.onloadend = function () {
        ...
        db.collection("orders").doc(orderId).set({
            orderId: orderId,
            name: custName,
            phone: "+91" + custPhone, // Automatically adds the +91 for your database
            email: document.getElementById("custEmail").value || "N/A",
            address: "Stall Pickup (BBAU)",
            total: total,
            items: cart,
            paymentImage: base64Image,
            status: "pending",
            eventType: "BBAU Mobile Order",
